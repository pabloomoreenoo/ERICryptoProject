...
const contractAddress = '0xf6d476D3C98aB95f2b900c151B4BB37caA2cbddA'  // contrato wavePortal

// estado de sesión (token devuelto por tu backend tras verificar firma)
const sessionToken = ref<string | null>(
  typeof window !== 'undefined' ? localStorage.getItem('sessionToken') : null
)

const emailVerified = computed(() => !!sessionToken.value)
...
async function checkIfWalletIsConnected() {
  try {
    ...
    // obtenemos cuentas ya autorizadas
    const accounts = await ethereum.request({ method: 'eth_accounts' })
    if (Array.isArray(accounts) && accounts.length > 0) {
      account.value = accounts[0]
      console.log('Cuenta detectada on load:', account.value)
      // carga los datos que quieras si hay cuenta
      await getWaveCount()
      await getAllWaves()
      await getBalance()
    } else {
      console.log('No hay cuentas autorizadas (eth_accounts returned none)')
    }
  } catch (err) {
    console.error('checkIfWalletIsConnected error', err)
  }
}

// helpers para set/clear
function setSessionToken(token: string) {
  sessionToken.value = token
  if (typeof window !== 'undefined') localStorage.setItem('sessionToken', token)
}
